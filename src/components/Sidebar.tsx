...
export function Sidebar({ currentUser, onOpenAuth, appTheme, onToggleTheme, posts, onHashtagClick }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex w-[260px] border-r border-white/5 h-screen sticky top-0 bg-[#0c0a13] flex-col items-stretch px-6 py-8 z-20 shrink-0">
      ...
      <div className="flex md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#0c0a13]/95 backdrop-blur-lg border-t border-white/5 px-4 items-center justify-around z-40 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
        ...
        <button 
          onClick={onOpenAuth}
          className="p-2 rounded-xl text-slate-500 hover:text-purple-400 transition-colors cursor-pointer"
          title="Login"
        >
